...
const DAILY_DIGEST_KEY = "pulsepocket:dailyDigest";

export const SWAP_AMOUNT_OPTIONS = [0.01, 0.02, 0.05, 0.1, 1] as const;
export const DEFAULT_SWAP_AMOUNT_SOL = 0.02;
export const SWAP_AMOUNT_MIN_SOL = 0.001;
export const SWAP_AMOUNT_MAX_SOL = 10;
...
export async function loadSettings(): Promise<AppSettings> {
  const [swapRaw, notifsRaw, digestRaw] = await Promise.all([
    AsyncStorage.getItem(SWAP_AMOUNT_KEY),
    AsyncStorage.getItem(NOTIFS_KEY),
    AsyncStorage.getItem(DAILY_DIGEST_KEY),
  ]);
...
export async function saveDailyDigest(enabled: boolean): Promise<void> {
  await AsyncStorage.setItem(DAILY_DIGEST_KEY, enabled ? "1" : "0");
}
